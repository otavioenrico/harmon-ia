...
- [x] Fase 3 — commit: `c29564f`. CSP (`script-src` sem `unsafe-inline`: hash do snippet `.js` repetido em 10 páginas + `esm.sh` p/ import do supabase-js; os 2 inline scripts reais viraram `assets/js/entrar.js` e `assets/js/oauth-redirect.js`; `style-src 'unsafe-inline'` mantido — app usa `style=` inline em 10+ módulos JS, proibir exigiria reescrever a renderização), HSTS, X-Frame-Options, Permissions-Policy em `vercel.json`. `site.webmanifest` linkado nas 11 páginas (ícones 192/512 já existentes). `.sr-only` do `entrar.html` migrado p/ `components.css` (mais um `<style>` inline eliminado). `waitlist.js` virou auto-invocado (elimina 2 wrappers inline em index/planos). `preconnect` p/ Supabase/esm.sh/Google em index/entrar/app.html. `hero-banner.png` conferido: nunca foi rastreado pelo Git (já no `.gitignore`), nada a remover; `credenciais/` gitignorado, sem segredo versionado. Testado local via `vercel dev` (headers confirmados por `curl -I`) + login Google real ponta a ponta pelo browser: zero violação de CSP no console.
